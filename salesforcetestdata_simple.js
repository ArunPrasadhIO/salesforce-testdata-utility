...
// Generate wide CSV for performance testing with proper field mapping
app.post("/generate-wide-csv", async (req, res) => {
  try {
    const objectName = req.body.objectName || 'TestObject__c';
    const recordCount = parseInt(req.body.recordCount) || 10;
    const requestedFieldCount = parseInt(req.body.fieldCount) || 200;
    
    console.log(`Generating wide CSV with real field mapping for ${objectName}: ${recordCount} records`);
    
    // Connect to Salesforce to get actual schema
    const { accessToken, instanceUrl } = await loggingin();
    const conn = new jsforce.Connection({
      instanceUrl: instanceUrl,
      accessToken: accessToken,
    ...
    // Get object schema with all fields
    const objectDescription = await conn.sobject(objectName).describe();
    
    // Filter to get all createable custom fields (not just 20)
    const allFields = objectDescription.fields
      .filter(field => 
        field.createable && 
        !field.autoNumber && 
        !field.calculated && 
        field.name !== 'Id' &&
        field.name !== 'OwnerId' // Skip reference fields that cause issues
      );
    
    // Use actual field count or requested count, whichever is smaller
    const actualFieldCount = Math.min(allFields.length, requestedFieldCount);
    const fields = allFields.slice(0, actualFieldCount);
    
    console.log(`Using ${fields.length} actual fields from ${objectName} (requested: ${requestedFieldCount})`);
    
    // Generate CSV headers with real field names
    const headers = fields.map(field => field.name);
    const csvRows = [headers.join(',')];
    
    // Generate sample data based on actual field types
    for (let record = 1; record <= recordCount; record++) {
      const row = fields.map(field => {
        switch (field.type) {
          case 'string':
            return `"Text Data ${record} for ${field.label}"`;
          case 'textarea':
            return `"Long text content ${record} for ${field.label} with detailed information and multiple sentences to fill the textarea field properly."`;
          case 'email':
            return `"record${record}@${field.name.toLowerCase().replace('__c', '')}.com"`;
          case 'phone':
            return `"555-${(1000 + record).toString().padStart(4, '0')}"`;
          case 'url':
            return `"https://example${record}.com/${field.name.toLowerCase()}"`;
          case 'int':
          case 'double':
          case 'currency':
          case 'percent':
            return (Math.random() * 1000 + record * 50).toFixed(2);
          case 'boolean':
            return Math.random() > 0.5 ? 'true' : 'false';
          case 'date':
            const date = new Date();
            date.setDate(date.getDate() + record);
            return date.toISOString().split('T')[0];
          case 'datetime':
            const datetime = new Date();
            datetime.setHours(datetime.getHours() + record);
          ...
          default:
            return `"Sample ${field.label} ${record}"`;
        }
      });
    ...
    res.json({
      ...
      estimatedUploadTime: `${Math.round(dataPoints / 100)} seconds (estimated)`,
      fieldTypes: fields.reduce((acc, field) => {
        acc[field.type] = (acc[field.type] || 0) + 1;
        return acc;
      }, {}),
      csvPreview: csvRows.slice(0, 3).join('\\n')
    });

  } catch (error) {
    console.error(`Error generating wide CSV: ${error.message}`);
    res.status(500).send(error.message);
  }
});
  ...
  try {
    ...
    const fields = objectDescription.fields
      .filter(field => 
        ...
        field.name !== 'Id' &&
        field.name !== 'OwnerId' // Skip reference fields that cause issues
      ); // Use ALL available fields for complete coverage
...
app.post("/bulk-upload", upload.single('csvFile'), async (req, res) => {
  try {
    ...
    const fieldCount = Object.keys(csvData[0] || {}).length;
    let batchSize;
    
    if (fieldCount > 100) {
      batchSize = 5;   // Very wide records: smaller batches
      console.log(`High field count detected (${fieldCount} fields): Using small batch size of ${batchSize}`);
    } else if (fieldCount > 50) {
      batchSize = 10;  // Wide records: medium batches  
      console.log(`Medium field count detected (${fieldCount} fields): Using medium batch size of ${batchSize}`);
    } else {
      batchSize = 50;  // Normal records: larger batches
      console.log(`Normal field count detected (${fieldCount} fields): Using standard batch size of ${batchSize}`);
    }
    ...
    for (let i = 0; i < csvData.length; i += batchSize) {
      ...
      console.log(`Processing batch ${batchNum}/${totalBatches} (${batch.length} records with ${fieldCount} fields each)`);
      ...
      try {
        ...
        // Add smaller delay for high field count scenarios
        if (i + batchSize < csvData.length) {
          const delay = fieldCount > 100 ? 500 : 1000; // Shorter delay for wide records
    ...
    console.log(`Total upload completed in ${totalTime}ms (${Math.round(totalTime/1000)}s)`);
    console.log(`Performance: ${Math.round((csvData.length * fieldCount) / (totalTime/1000))} data points per second`);
